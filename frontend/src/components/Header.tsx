  ...
  return (
    <header className="absolute w-full z-50 py-6">
      <nav className="container mx-auto px-6 flex justify-between items-center relative">
        ...
        >
          {navItems.map((item) => (
            <Link key={item.name} to={item.path} className="text-white hover:text-blue-300 transition-colors duration-300">
              {item.name}
            </Link>
          ))}
        </motion.div>
      ...
      {menuOpen && (
        ...
        >
          ...
          <Link to="/get-started" className="bg-white text-blue-600 px-6 py-2 rounded-full hover:bg-blue-100 transition-colors duration-300">
            Get Started
          </Link>
        </motion.div>
      )}
